use flotilla_protocol::{Command, WorkItem, WorkItemKind, RepoLabels};
use super::App;
...
impl Intent {
    ...
    pub fn resolve(&self, item: &WorkItem, app: &App) -> Option<Command> {
        match self {
            ...
            Intent::OpenPr => {
                item.pr_key.as_ref().map(|k| Command::OpenPr { id: k.clone() })
            }
            Intent::OpenIssue => {
            ...
            Intent::ArchiveSession => {
                item.session_key.as_ref().map(|k| Command::ArchiveSession { session_id: k.clone() })
            }
        }
    }
