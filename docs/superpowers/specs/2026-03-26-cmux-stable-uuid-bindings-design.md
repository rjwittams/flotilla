...
**Reverse lookup strategy — newest wins:** Multiple workspace bindings can exist for the same attachable set (e.g., stale binding from a destroyed workspace alongside the current one, or in future multiple workspaces observing the same set). The reverse lookup (`lookup_workspace_ref_for_set`) uses `.rfind()` to return the newest (last-appended) binding. The call site filters the result by the provider's `binding_scope_prefix()` to ensure only bindings from the current "universe" (active cmux instance, current zellij session, live tmux server) are considered. Stale bindings are cleaned up by the pruning logic during refresh. This avoids a 1:1 constraint that would need to be unwound when we add multi-workspace observation of the same set.
